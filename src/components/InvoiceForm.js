...
class InvoiceForm extends React.Component {
  ...
  handleCalculateTotal() {
    ...
    const discountAmmount = parseFloat(subTotal * (discountRate / 100)).toFixed(
      2
    );
  ...
  onItemizedItemEdit(evt) {
    ...
    const newItems = this.state.items.map((currentItem) => {
      const updatedItem = { ...currentItem };
      for (let key in updatedItem) {
        if (key === item.name && updatedItem.id === item.id) {
          updatedItem[key] = item.value;
        }
      }
      return updatedItem;
    });
